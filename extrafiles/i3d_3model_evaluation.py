...
def load_flow_video(path, max_frames=0, resize=(224, 224)):
  print("converting RGB to optical flow ...")
  cnvstrtime = time.time()
  cap = cv2.VideoCapture(path)
  ret, frame1 = cap.read()
  ...
  try:
    ...
    cap.release()
    cnvtime = time.time() - cnvstrtime
    # print('Num of frames converted : ', frames.shape[0])
    print('conversion time : ', cnvtime)
    
    # print((np.array(frames) / 255.0).shape)
  return (np.array(frames) / 255.0),cnvtime
...
if __name__ == '__main__':
  ...
  parser.add_argument('--eval_type',type=str,default='joint',help='Input the required evluation type(rgb, flow, or joint')
  ...
  for eval_type in eval_types:
    print('Model in use is :', eval_type)
    ...
    if eval_type in ['rgb', 'rgb600']:
      ...
      rgbinput = load_video(args.video)
    ...
    if eval_type in ['flow']:
      ...
      flowinput,cnvtime=load_flow_video(args.video)
      print('flow video shape :', flowinput.shape)
      print('flow number of frames :', flowinput.shape[0])
      print('average conversion time :',cnvtime/flowinput.shape[0])
    ...
    with tf.Session() as sess:
      ...
      # print_video(args.video,out_predictions[topaction]*100,kinetics_classes[topaction])
      # cv2.imshow()
      accuracy.append(out_predictions[topaction]*100) 

      # for index in sorted_indices[:10]:
        # print(index,' | ',out_predictions[index] * 100,'  | ', out_logits[index],'  | ', kinetics_classes[index])
      print(topaction,' | ',out_predictions[topaction] * 100,'  | ', out_logits[topaction],'  | ', kinetics_classes[topaction])
  
    print("\n calculation time taken :",entime-sttime)
    calctime.append(entime-sttime)

  plt.bar(eval_types, accuracy, align='center', alpha=0.5)
  # plt.yticks(eval_types, accuracy)
  plt.ylabel('Accuracy %')
  plt.xlabel('Models')
  plt.title('Accuracy Evaluation')
  plt.show()

  plt.bar(eval_types, calctime, align='center', alpha=0.5)
  # plt.yticks(eval_types, accuracy)
  plt.ylabel('Time in seconds')
  plt.xlabel('Models')
  plt.title('Calculation Time Evaluation')
